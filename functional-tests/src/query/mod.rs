pub mod add_attribute_to_category;
pub mod add_base_product_to_coupon;
pub mod add_in_cart;
pub mod add_in_cart_v2;
pub mod add_package_to_company;
pub mod apply_password_reset;
pub mod change_password;
pub mod clear_cart;
pub mod create_attribute;
pub mod create_attribute_value;
pub mod create_base_product;
pub mod create_base_product_with_variants;
pub mod create_category;
pub mod create_coupon;
pub mod create_custom_attribute;
pub mod create_delivery_company;
pub mod create_package;
pub mod create_product;
pub mod create_store;
pub mod create_store_subscription;
pub mod create_user;
pub mod create_user_delivery_address_full;
pub mod create_warehouse;
pub mod create_wizard_store;
pub mod deactivate_base_product;
pub mod deactivate_product;
pub mod deactivate_store;
pub mod deactivate_user;
pub mod delete_attribute;
pub mod delete_attribute_from_category;
pub mod delete_attribute_value;
pub mod delete_category;
pub mod delete_company_package;
pub mod delete_coupon;
pub mod delete_custom_attribute;
pub mod delete_delivery_company;
pub mod delete_from_cart;
pub mod delete_package;
pub mod delete_store;
pub mod delete_user;
pub mod delete_user_delivery_address_full;
pub mod delete_warehouse;
pub mod delete_wizard_store;
pub mod generate_coupon_code;
pub mod get_attributes;
pub mod get_available_shipping_for_user;
pub mod get_base_product;
pub mod get_cart;
pub mod get_categories;
pub mod get_categories_with_products;
pub mod get_company_package;
pub mod get_existing_reset_token;
pub mod get_jwt_by_email;
pub mod get_jwt_by_provider;
pub mod get_me;
pub mod get_package;
pub mod get_store;
pub mod get_store_subscription;
pub mod get_user_delivery_addresses_full;
pub mod get_wizard_store;
pub mod increment_in_cart;
pub mod introspection;
pub mod refresh_jwt;
pub mod remove_delivery_method_from_cart;
pub mod request_password_reset;
pub mod revoke_jwt;
pub mod send_base_product_to_moderation;
pub mod send_store_to_moderation;
pub mod set_comment_in_cart;
pub mod set_coupon_in_cart;
pub mod set_delivery_method_in_cart;
pub mod set_moderation_status_base_product;
pub mod set_moderation_status_store;
pub mod set_quantity_in_cart;
pub mod set_selection_in_cart;
pub mod update_attribute;
pub mod update_attribute_value;
pub mod update_base_product;
pub mod update_category;
pub mod update_coupon;
pub mod update_delivery_company;
pub mod update_package;
pub mod update_product;
pub mod update_store;
pub mod update_store_subscription;
pub mod update_user;
pub mod update_user_delivery_address_full;
pub mod update_warehouse;
pub mod update_wizard_store;
pub mod upsert_shipping;
pub mod verify_email;
